...
The results of the first model (@tbl-tab3) indicates that a longer track length is associated with a `r exp(logit1.2)` higher likelihood of being a song. When we add in the other covariates (Model 2, @tbl-tab4), the likelihood of a track being a song is `r exp(logit2.2)` times higher when a track is 1 minute longer.
...
glm(song ~  length, data = data, family = "binomial") %>%
  tidy() %>%
    mutate(
...
#| label: tbl-tab4
#| tbl-cap: "Results of logistic regression for the likelihood of a track being a song"
glm(song ~  length + playlist_genre + tempo + loudness, data = spotify_songs, family = "binomial") %>%
  tidy() %>%
    mutate(
    p.value = scales::pvalue(p.value),
...
glm(liv_acoust ~  length, data = spotify_songs, family = "binomial") %>%
  tidy() %>%
    mutate(
    p.value = scales::pvalue(p.value),
    term = c("Intercept", "Song Length")
...
#| label: tbl-tab6
#| tbl-cap: "Results of logistic regression for the likelihood of a track being a song"
glm(liv_acoust ~  length + playlist_genre + tempo + loudness, data = spotify_songs, family = "binomial") %>%
  tidy() %>%
    mutate(
    p.value = scales::pvalue(p.value),
    term = c("Intercept", "Song Length", "Latin", "Pop", "R&B", "Rap", "Rock", "Tempo", "Loudness")
